...
Connection registration reads each login’s own session attributes. MySQL 8.4 exposes `session_account_connect_attrs` through a world-readable, account-filtered Performance Schema table; ephemeral logins need no delegated grant on it. The persistent-role script includes explicit `SELECT` for clarity, not as an isolation boundary. Boot also needs `XA_RECOVER_ADMIN` to refuse recovery when prepared XA transactions exist. This is server-wide visibility; comms does not resolve unknown transactions. Neither login receives `PROCESS` or `SUPER`, and backups use `--no-tablespaces`.
