...
it("requires a fresh human proof bound to the observed lock and preserves a replacement on replay", async (test) => {
	...
	expect(
		(
			await fetch(`${app.url}/_boot/enroll/${enrollment.id}/approve`, {
				method: "POST",
				headers: { ...human, "x-comms-assertion": await proof("enrollment.decide", decision) },
	...
	const signedHuman = { ...human, "x-comms-assertion": signed };
	for (const headers of [
		{ cookie: session.cookie, "content-type": "application/json", "x-comms-assertion": signed },
	...
	for (const headers of [
		...
		expect((await breakLock(lock.id, { ...headers, "x-comms-assertion": signed })).status).toBe(401);
	}
	expect(
		(
			await breakLock(lock.id, {
				...human,
				"x-comms-assertion": await proof("token.revoke", { family: lock.holder_family }),
			})
		).status,
	).toBe(401);
	...
	expect((await breakLock(lock.id, { ...human, "x-comms-assertion": stale })).status).toBe(423);
	expect(await inspect()).toEqual(replacement);
});
