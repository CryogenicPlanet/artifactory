...
it("binds token mint to a human session, exact Origin, fresh verified passkey, and canonical request", async (test) => {
	...
	for (const [supplied, status] of [
		...
		[{ ...headers, "x-chirp-auth-kind": "human", "x-chirp-agent": "rahul", cookie: "" }, 401],
	...
	expect((await mint(app.url, { ...headers, "x-chirp-assertion": otherAction })).status).toBe(401);
	for (const [origin, rpId, uv] of [
		...
		expect((await mint(app.url, { ...headers, "x-chirp-assertion": bad })).status).toBe(401);
	}
	const signed = await env.proof(app, cookie, "token.mint", input);
	const authorized = { ...headers, "x-chirp-assertion": signed };
	...
	for (const supplied of [
		...
		expect(
			(await mint(app.url, { ...supplied, "content-type": "application/json", "x-chirp-assertion": signed })).status,
		).toBe(401);
		expect(
			(
				await fetch(`${app.url}/_boot/auth/challenge`, {
...
it("replays one exact signed mint concurrently and across restart, refreshes direct grants, and refuses revoked replay", async (test) => {
	...
	for (const long_lived of [false, true]) {
		...
		const headers = { ...human(cookie), "idempotency-key": key, "x-chirp-assertion": signed };
		...
		expect((await mint(app.url, { ...headers, "x-chirp-assertion": replacement }, body)).status).toBe(401);
		...
		const revoked = await fetch(`${app.url}/_boot/tokens/${pair.family}/revoke`, {
			method: "POST",
			headers: { ...human(cookie), "x-chirp-assertion": revokeProof },
			body: "{}",
		});
...
for (const mode of ["logout", "expiry"])
	it(`refuses a held mint body after session ${mode}`, async (test) => {
		...
		const pending = request(`${app.url}/_boot/tokens`, {
			method: "POST",
			headers: { ...human(cookie), "x-chirp-assertion": proof, "content-length": String(Buffer.byteLength(body)) },
		});
		test.onTestFinished(() => {
			pending.destroy();
		});
