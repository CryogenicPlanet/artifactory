...
import { backupRoute, type BackupCaptureStore, type BackupStore } from "../../src/backup-http.ts";
...
import type { BackupInventory } from "../../src/backup-inventory.ts";
...
const program = Effect.gen(function* () {
	...
	yield* Effect.gen(function* () {
		...
		yield* Effect.gen(function* () {
			const auth = yield* Ref.make<Auth["Service"] | null>(yield* Auth);
			const inventory: BackupStore = yield* Ref.make<BackupInventory | null>(null);
			const events = yield* Ref.make<Events["Service"] | null>(yield* Events);
			...
			for (const error of [
				...
				const capture: BackupCaptureStore = yield* Ref.make<DatabaseBackup | null>({
					capture: () => Effect.fail(error),
				});
				for (const internal of [false, true]) {
					...
					assert.ok(response);
					assert.equal(response.status, error.code === "unsafe_artifact_path" ? 409 : 507);
