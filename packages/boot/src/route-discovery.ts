...
const routes = [
	...
	[
		...
		["/_boot/settings"],
		"human",
		"Change {revision,patch} with exact Origin and a fresh settings.change X-Comms-Assertion bound to that body and session. patch may contain storage or public_paths. Calendar retention is retired: new event_retention changes are refused with invalid_request; only previously accepted exact signed retries remain readable. Repeat the exact proof and body after a lost response to read the first accepted result; it never reapplies over a later change.",
	],
	...
	[
		...
		["/_boot/db/restore"],
		"human",
		"Restore {backup} (or {id}) with exact Origin and a fresh db.restore X-Comms-Assertion. Optional Idempotency-Key binds retries.",
	],
	[
		"get",
	...
	[
		...
		["/_boot/lock", "/api/lock"],
		"fs",
		"Release your edit lock. ?break=1 instead requires a human session and fresh lock.break X-Comms-Assertion bound to the observed lock.",
	],
	[
		"post",
	...
	[
		...
		"proof",
		"Approve or deny with {decision,scopes,long_lived}, exact Origin and a fresh enrollment.decide X-Comms-Assertion. No prior session required.",
	],
	[
		"post",
	...
	[
		...
		"human",
		"Mint a token pair with exact Origin and a fresh token.mint X-Comms-Assertion. Optional Idempotency-Key must be bound in the challenge.",
	],
	[
		"post",
		["/_boot/tokens/{family}/revoke", "/api/tokens/{family}/revoke"],
		"human",
		"Revoke a family with {}, exact Origin and a fresh token.revoke X-Comms-Assertion.",
	],
	...
	[
		...
		"human",
		"Finish registration with {id,label,response}, exact Origin and a fresh passkey.add X-Comms-Assertion.",
	],
	[
		"delete",
		["/_boot/auth/passkeys/{id}"],
		"human",
		"Delete a passkey with {}, exact Origin and a fresh passkey.delete X-Comms-Assertion. The last key cannot be deleted.",
	],
		...
		components: {
			securitySchemes: {
				...
				commsBootAccess: {
					type: "http",
					scheme: "bearer",
					description: "Enrolled access token; required scopes are given by x-comms-scopes.",
				},
			},
		},
		init_url: "/init",
