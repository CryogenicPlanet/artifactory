...
Missing or foreign live-store repair still needs an offline preservation protocol before HTTP restore can replace it safely. The current restore path requires a verifiable live store for its safety backup and rollback. Preserve the selected file, sidecars, boot store and journals; select the matching current store and restart. Do not bypass identity checks or discard a pending reservation to enable restore.
