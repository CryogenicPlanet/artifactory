...
`topic.moved` is reserved for boot-coordinated topic relocation. `ctx.log` refuses that type before reserving a sequence. Returned HTTP handler Promises are drained on client cancellation: topic maintenance waits for their completion before moving the tree. A handler Promise that never settles prevents maintenance admission until boot can prove the old process is closed.
