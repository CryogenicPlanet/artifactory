...
for (const engine of ["pg", "mysql"] as const) {
	...
	it.skipIf(!boot || !app)(
		`projects real ${engine} boot and core catalogs including migration ledgers`,
		async () => {
			...
			try {
				const { stdout } = await promisify(execFile)(
					"bun",
					[join(import.meta.dirname, "../fixtures/transfer-native-catalog.ts"), boot, app, join(root, "boot.sqlite")],
					{ timeout: 30000, env: process.env },
				);
				expect(stdout).toContain(`CATALOG_VERIFIED ${engine} boot`);
				expect(stdout).toContain(`CATALOG_VERIFIED ${engine} app`);
			} finally {
