...
const initialize = (sql: SqlClient.SqlClient) =>
	on(sql, {
		sqlite: () =>
			sql`CREATE TABLE IF NOT EXISTS protected_sql_tables(name TEXT PRIMARY KEY COLLATE NOCASE)`.pipe(Effect.asVoid),
...
export const registerProtectedSqlTable = (sql: SqlClient.SqlClient, name: string) =>
	Effect.gen(function* () {
		...
		yield* sql`INSERT INTO protected_sql_tables(name) VALUES(${name.toLowerCase()}) ${on(sql, {
