			...
			.withTransaction(
				Effect.gen(function* () {
					...
					yield* publication.recordEvent(
						...
						(seq) =>
							sql`INSERT INTO kv(ns,${sql("key")},value,updated_seq,previous) VALUES('kernel-health',${marker},${marker},${seq},NULL)`.pipe(
								Effect.asVoid,
							),
					);
					const rows = yield* publication.read(
						(fence) =>
							sql`SELECT value FROM kv WHERE ns='kernel-health' AND ${sql("key")}=${marker} AND updated_seq<=${fence}`,
					);
