...
export const remoteWriteGuard = (
	...
	Effect.gen(function* () {
		...
		const executable = yield* (
			dialect === "pg"
				? sql`SELECT p.proname AS name FROM pg_catalog.pg_proc p JOIN pg_catalog.pg_namespace n ON n.oid=p.pronamespace WHERE n.nspname NOT IN ('pg_catalog','information_schema') AND NOT EXISTS(SELECT 1 FROM pg_catalog.pg_depend d JOIN pg_catalog.pg_extension e ON e.oid=d.refobjid WHERE d.classid='pg_proc'::regclass AND d.objid=p.oid AND d.deptype='e' AND e.extname='unaccent') AND NOT (n.nspname='public' AND p.proname='comms_unaccent' AND p.prokind='f' AND NOT p.proretset AND p.pronargdefaults=0 AND p.pronargs=1 AND p.provolatile='i' AND NOT p.prosecdef AND p.prorettype=25 AND p.proargtypes='25'::pg_catalog.oidvector AND p.proconfig=ARRAY['search_path=pg_catalog'] AND p.prolang=(SELECT oid FROM pg_catalog.pg_language WHERE lanname='sql') AND (p.prosrc='SELECT value' OR (p.prosrc=${"SELECT public.unaccent('public.unaccent'::pg_catalog.regdictionary, value)"} AND EXISTS(SELECT 1 FROM pg_catalog.pg_depend d JOIN pg_catalog.pg_extension e ON e.oid=d.refobjid WHERE d.classid='pg_proc'::pg_catalog.regclass AND d.objid=pg_catalog.to_regprocedure('public.unaccent(pg_catalog.regdictionary,text)') AND d.deptype='e' AND e.extname='unaccent')))) LIMIT 1`
