...
export const makeTopics = (sql: SqlClient.SqlClient, read: Messages["Service"]["read"], pages: Pages["Service"]) => {
   ...
   FROM visible_topics t WHERE NOT EXISTS(SELECT 1 FROM visible_topics a WHERE a.deleted_at IS NOT NULL AND (t.path=a.path OR ${isDescendant(sql, sql`t.path`, sql`a.path`)})) AND (${path}='' OR t.path=${path} OR substr(t.path,1,length(${path})+1)=${path + "/"})
