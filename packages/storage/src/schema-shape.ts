	...
	Effect.gen(function* () {
		...
		if (options.foreignKeys) {
			const foreign = yield* on(sql, {
				...
				pg: () =>
					sql`SELECT k.column_name AS "column",f.table_name AS "table",f.column_name AS target,CASE WHEN f.table_schema='public' AND EXISTS(SELECT 1 FROM pg_catalog.pg_constraint v JOIN pg_catalog.pg_namespace vn ON vn.oid=v.connamespace JOIN pg_catalog.pg_class vt ON vt.oid=v.conrelid WHERE vn.nspname=c.constraint_schema AND vt.relname=c.table_name AND v.conname=c.constraint_name AND v.convalidated) AND r.update_rule IN ('NO ACTION','RESTRICT') AND r.delete_rule IN ('NO ACTION','RESTRICT') THEN 1 ELSE 0 END AS valid FROM information_schema.table_constraints c JOIN information_schema.key_column_usage k ON k.constraint_schema=c.constraint_schema AND k.constraint_name=c.constraint_name JOIN information_schema.referential_constraints r ON r.constraint_schema=c.constraint_schema AND r.constraint_name=c.constraint_name JOIN information_schema.key_column_usage f ON f.constraint_schema=r.unique_constraint_schema AND f.constraint_name=r.unique_constraint_name AND f.ordinal_position=k.position_in_unique_constraint WHERE c.table_schema='public' AND c.table_name=${table} AND c.constraint_type='FOREIGN KEY' ORDER BY k.column_name`,
				mysql: () =>
