...
Construct the inspector scope before the client and provide its service to the client layer. Every physical lease must receive a durable registration acknowledgement before application SQL, including transactions, streams, and explicit reservation. MySQL leases must also report `REPEATABLE-READ` session isolation; other levels fail with `remote_isolation_unsupported` before registration. Admission checks the leased connection and does not change operator settings. The callback must use independent storage or IPC, never this pool. The inspector checks the same pinned connection before acknowledging registration. Failed registration releases its lease; the patched MySQL driver also owns its pool before its initial connectivity query can fail.
