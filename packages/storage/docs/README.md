...
- `render` returns an Effect that validates file paths and produces a `Redacted` descriptor. Unwrap only at a required connection/environment boundary. Boot credentials must never reach editable children. The SQLite-only `parse` and `childStore` exports retain the file/`APP_DATABASE` compatibility contract, including alias-only startup for older images; remote callers use `parseDescriptor`.
