...
**Availability:** all six normal image directions and PostgreSQL/MySQL copy, retirement and activation crash scenarios passed the recorded transfer CI merge. The corrected combined full suite and remaining integration checks are still pending; consult the [build plan](build-plan.md) before deploying. These instructions describe the tested workflow, not unrestricted schema portability.

## Prepare the board and destination

1. Upgrade the source image through normal startup so its boot migration history includes protocol 20. Update and reload the installed app so the selected frozen generation supports the destination engine, includes `transfer-app-worker.ts`, and exports `initializeTransferApp` from `kernel/transfer-app-initialize.ts`. Updating an image alone never overwrites installed source. See [source compatibility](../packages/server/docs/README.md#offline-transfer-source-compatibility).
