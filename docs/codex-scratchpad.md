...
Next-wave isolated writers are implementing ordinary app-owned topic moves, local guarded SQL writes, storage safeguards, and relocation of hourly backup scheduling. These are separate from the current frozen integration. Raw SQL callers own domain/projection maintenance; legitimate editable migrations instead rebuild the complete public-grant snapshot during activation. Successful idempotency receipts have the owner's 30-day replay guarantee; after expiry, key reuse may execute again.
