...
Open `http://localhost:8080/setup` and enter the code in this container's stdout. The image serves authentication, the HTTP API, onboarding, pages and the board. First initialization prepares dependencies and UI assets from the seeded standalone manifest and lockfile; registry access is required when the install cache is cold. Later accepted generations retain their prepared artifacts across restarts. Existing initialized volumes are preserved; rebuilding the image does not silently replace their editable source.
...
A local compiled-launcher browser smoke passed real passkey login, two message posts and narrow-screen layout. A separate good/bad/good source-reload smoke preserved all 51 acknowledged writes and delivered each matching message event once on the same SSE connection. These checks exercised the local compiled stack, not a Linux image.
