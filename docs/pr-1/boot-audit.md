...
- **`GET /_boot/agents`** (`event-http.ts:147`). Entirely derivable from `X-Comms-Agent`, `X-Comms-Instance` and `X-Comms-Label`, which `proxy.ts:299-307` forwards on every authenticated request, plus the `last_seen_at` update §6 already says happens on every request. Delete it; `GET /api/agents` becomes an extension over the app's own table.
