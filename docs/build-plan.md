...
The owner revised `docs/database.md` and added `docs/pr-1/stack-review-2b3e365.md`; neither is an agent edit. The other 15 original protected files match the takeover inventory. Current preservation inventory: `/tmp/comms-base-owner-hashes-854e18c.json` (17 files).
...
## Base review audit at `854e18c`

These are open requirements, not waived suggestions. Isolated writers are completing them before the database stack resumes:

- Item 19/21: shared boot-route preamble and centralized Origin policy, preserving route-specific authentication order, queued reauthorization and request-scope finalizers.
- Item 15: cancelable, bounded SQL statement execution; the existing whole-child watchdog alone does not satisfy the requested statement interruption.
- Item 20: remove remaining redundant handwritten UI component styles (262 lines across the two current stylesheets); retain rendered-Markdown appearance with existing Tailwind utilities and minimal browser verification.
- Item 21: structural boot reduction remains incomplete; the approximately 7,250-line estimate is not met by the current 12,158 lines.
- Acceptance: exact-head Linux and QEMU results remain required. QEMU at `854e18c` failed before reboot on HTTP507; diagnostic-only `9ad07fa` captures bounded allowlisted storage evidence without retries or deadline changes.

Historical reaction rows are retained for data compatibility although their product routes/UI are removed. The latest stack review's claims that restart, metrics, combined restore, the startup layer graph and restore gate finalization are absent are superseded by current code; its sequencing objection is valid. The SQL HTTP adapter remains a kernel route as explicitly listed in SPEC §6, while core domain routes use extension capabilities.
