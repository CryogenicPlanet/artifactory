...
**Base first:** PRs #2–4 and prepared migration/remote-client work remain paused and unintegrated. No merge or deployment is implied. Root `66b1a9c` is pushed. It includes `e96dec5` removing the obsolete selector-only undo coordinator, `75fa8f1` consolidating traffic resume under supervisor activation, `a834e3f` moving live discovery composition into the editable server, and the ownership audit. Source inspection/revert remains available; lifecycle cleanup alone is not a claim that convenience responsibilities have been removed.

**Latest completed local acceptance:** production `a834e3f` passes root check and the full suite with actual Node 22.22.3 and **two workers**: **782 tests passed, one opt-in skipped**, across **191 passed files and one skipped file**, in **519.49 seconds**. This accepts the pushed production baseline, not the new isolated scope changes. Linux/image/QEMU for the new pushed head remain pending; consult the PR checks for exact results.

**Active isolated writers — not integrated:** `boot_diagnostics_scope` narrows public boot events while keeping the protected internal append/query and publication evidence needed by the app. `boot_telemetry_scope` removes optional Prometheus presentation and child-span aggregation while preserving minimal bounded diagnostics, verified attribution, request IDs, redaction and private-header security. Neither worker report nor these planned removals is combined-checkout acceptance. Line-count estimates, including the former 250-line proposal, are not acceptance criteria.
...
1. Integrate the independently reviewed public-event diagnostic separation and telemetry removal against the current baseline. App browsing must remain app-owned; internal publication evidence and minimal dead-app failure diagnostics remain protected.
2. Verify authentication, event filtering/cursors, internal append/query authority, private-header stripping and broken-app diagnostics with focused behavior tests. Review source version inspection and safe revert for unchanged availability and durable replay.
3. Follow the audit’s next scope decisions in order: conditional raw writes before retiring anchored editing; no-reseed compatibility before moving page-content initialization; explicit retained-receipt/credential handling before retiring optional calendar deletion, duplicate log mirroring or manual token issuance. None is claimed implemented by this ledger.
4. Freeze the resulting combined code, run root check/build and the full suite with actual Node 22.22.3 and two workers, and revisit earlier workflow/measurement evidence where the changed boundaries require it. Keep UI verification minimal.
5. Commit/push coherent verified changes to the existing base PR and inspect exact-head Linux, image and QEMU. Report remaining ownership/spec gaps honestly; retain source version inspection and safe revert, and keep the database stack paused until the base is finished.
...
| Request tracing and Logger export | Historical cross-process aggregation and export were integrated and tested. The current audit intentionally removes child annotation aggregation and optional Prometheus presentation; that isolated work is not yet integrated. Minimal boot diagnostics and header security remain required. |
