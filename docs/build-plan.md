...
**Base first:** PRs #2–4 and prepared migration/remote-client work remain paused. Current local root `23b9be5` includes the ownership scope and two subsequent fixes. Owner commit `1013524` records the scope decisions in SPEC, tech and review comments; those owner documents are not edited by this ledger. `7064136` makes the stream fixture hold an explicit gate instead of relying on a timing race (15/15 focused tests). `23b9be5` handles health-timeout retirement through the proven-closure failure path: the regression observed `frozen:true` on the baseline and passes after the fix; its focused group passes 23/23. Root combined check/build pass for these changes (`/tmp/comms-playable-check.log`, `/tmp/comms-playable-build.log`); new-head CI remains pending. No full-suite acceptance of this newer production fix is claimed.
...
1. Retain passing combined check/build and complete appropriate full-suite acceptance for the latest stream-fixture and health-timeout changes. Preserve earlier passing and failing evidence; the 39e0bd0 full pass predates the production fix.
2. Confirm the fresh review’s retained boundaries: fs-only boot diagnostics, trusted event provenance, independent diagnostic cursor, app-owned rich queries, private compatibility, safe conditional writes, source inspection/revert and header stripping.
3. Integrate reviewed documentation into the existing base PR and inspect exact-head Linux, image and QEMU. The completed 39e0bd0 CI is historical; record subsequent results against their exact heads.
4. Keep page seeding unchanged without a safe no-reseed launch design. Calendar deletion/log mirroring/manual mint retirement remain separate audit proposals; they do not block this agreed slice. Keep the database stack paused until base acceptance and report remaining proposals honestly.
