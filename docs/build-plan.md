...
**The combined checkout passes 311 tests across 90 files, repository checks and all package builds.** This includes the private child-control guard, durable webhook delivery, cached storage measurements and bounded inbox/onboarding work. Idle inbox waits rescan messages only when the publication fence changes. The page-write admission regression also has a deterministic pending-publication test; it waits without blocking outbox progress and never retries an admitted publication. A fresh compiled smoke preserved all **49 acknowledged writes** across good/bad/good reloads, delivered each once over the same uninterrupted SSE connection, and preserved all 49 after a graceful boot restart. It also verified a new post/restart read, repaired extension response, served board JavaScript, prepared dependency/UI artifacts, passkey enrollment and staging isolation, with zero browser errors. Fresh review of the combined integration boundaries is clear. Real Linux/image acceptance is separate from these local results.

The latest review decisions in SPEC §12 (automatic read marks, top-level extension routes and removing core reactions) are being implemented in separate work streams; the integrated behavior below describes the current code.
...
| C. Conversation | Message CRUD/wait and filters; topics/unread/read marks; selectable inbox; search/reactions; metadata/archive/deletion; profiles/presence; bounded README/priorities/subtopic/inbox/activity context | Topic move; no undelete/physical-erasure endpoint |
...
| E. Cutover and recovery | WAL-inclusive rehearsal, prewarming, freeze/drain, synced pre-flip backup, durable acceptance/rollback, keeper receipts, Linux kernel boot-id checks, hourly backups, human backup inventory and weekly scratch-store drills | Public restore, snapshot/backup/artifact pruning, disk headroom/budgets, actual Linux reboot and ownership verification |
