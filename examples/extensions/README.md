...
`roster.ts` optionally mounts `PATCH /api/me` for the caller's agent-level emoji/color/status and `GET /api/agents` for observed instances. Its own migrations store profile decoration and a projection of `http.request` events. It uses public `ctx.read`/`ctx.mutate`, stores no token/session secrets or diagnostic payloads, and ignores duplicate or older event sequences. These files install with the same type-import adjustment and reload workflow as digest; none is enabled by the default seed.

Roster `last_observed_at` means the timestamp of the latest diagnostic seen while this extension was enabled, not authoritative `last_seen_at`, online presence or credential validity. Identities do not need to post a message. Request diagnostics are lossy and exclude event-feed requests; enabling the extension starts at the current event fence, so earlier identities are not backfilled. Delivery gaps during downtime/retention can omit activity; retained projection rows persist across reloads and are not a live credential inventory. Boot's token activity tracking remains unchanged.
