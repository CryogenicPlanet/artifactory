...
```sh
curl -H "Authorization: Bearer $ACCESS" "$HOST/api/digest?topic=project"
curl -H "Authorization: Bearer $ACCESS" "$HOST/api/digest?topic=project&mentions=@codex/job-17,@here"
```
